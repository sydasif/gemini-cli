...
def perform_research(query: str, output_format: str = "text", allowed_tools: str = "google_web_search") -> str:
    # Use shutil to find the executable in the system PATH
    gemini_bin = os.environ.get("GEMINI_BIN") or shutil.which("gemini")

    if not gemini_bin:
        raise FileNotFoundError("The 'gemini' executable was not found in PATH. Please install the Gemini CLI.")
    ...
    try:
        ...
        raise RuntimeError(f"Gemini CLI error (Exit {e.returncode}): {error_msg}")
